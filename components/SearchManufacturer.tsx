...
const SearchManufacturer = ({ manufacturer, setManufacturer }: SearchManufacturerProps) => {
  ...
  const filterManufacturers = query === "" ? manufacturers : manufacturers.filter((item) => item.toLowerCase().replace(/\s+/g, "").includes(query.toLowerCase()))
            ...
            <Combobox.Options className="search-manufacturer__options">
              {
                filterManufacturers.length === 0 && query !== "" ? (
                  ...
                  filterManufacturers.map(item => <Combobox.Option key={item} className={({ active }) => `relative search-manufacturer__option ${active ? 'bg-primary-blue text-white' : 'text-gray-900'}`} value={item}>
                    {item}
                  </Combobox.Option>)
                )
              }
